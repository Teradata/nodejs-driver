...
export class ExampleEncryptPassword {

  private sTransformation: string;
  private sKeySizeInBits: string;
  private sMac: string;
  private sPassKeyFileName: string;
  private sEncPassFileName: string;
  private sHostname: string;
  private sUsername: string;
  private sPassword: string;
  private sAlgorithm: string;
  private sMode: string;
  private sPadding: string;
  private match: string;

  private nCipherBlockSizeInBytes: number;
  private nKeySizeInBits: number;
  ...
  public setupAndRun(): void {
    this.setup();
    this.execute();
    this.decryptPassword(this.sPassKeyFileName, this.sEncPassFileName);
  }
  ...
  private setup(): void {
    ...
    this.sTransformation = process.argv[2];
    this.sKeySizeInBits = process.argv[3];
    this.sMac = process.argv[4];
    this.sPassKeyFileName = process.argv[5];
    this.sEncPassFileName = process.argv[6];
    this.sHostname = process.argv[7];
    this.sUsername = process.argv[8];
    this.sPassword = process.argv[9];

    const asTransformationParts: string[] = this.sTransformation.split('/');
    if (asTransformationParts.length !== 3) {
      console.log('Invalid transformation: ' + this.sTransformation);
      process.exit(1);
    }

    this.sAlgorithm = asTransformationParts [0];
    this.sMode      = asTransformationParts [1];
    this.sPadding   = asTransformationParts [2];

    if (['DES', 'DESede', 'AES'].indexOf(this.sAlgorithm) < 0) {
        console.log('Unknown algorithm ' + this.sAlgorithm);
        process.exit(1);
    }

    if (['CBC', 'CFB', 'OFB'].indexOf(this.sMode) < 0) {
      console.log('Unknown mode ' + this.sMode);
      process.exit(1);
    }

    if (['PKCS5Padding', 'NoPadding'].indexOf(this.sPadding) < 0) {
      console.log('Unknown padding ' + this.sPadding);
      process.exit(1);
    }

    if (['HmacSHA1', 'HmacSHA256'].indexOf(this.sMac) < 0) {
      console.log('Unknown MAC algorithm ' + this.sMac);
      process.exit(1);
    }
    ...
    this.nKeySizeInBits = parseInt(this.sKeySizeInBits, 10);
    this.match = Date.now().toString();

    if (this.sAlgorithm === 'AES') {
      this.nCipherBlockSizeInBytes = 16;
    } else {
      this.nCipherBlockSizeInBytes = 8;
    }
  }

  private execute(): void {

    const encryptKeys: any[] = this.createPasswordEncryptionKeyFile(
      this.sTransformation,
      this.sAlgorithm,
      this.match,
      this.sMac,
      this.nKeySizeInBits,
      this.sPassKeyFileName);

    this.createEncryptedPasswordFile(
      this.sTransformation,
      this.sAlgorithm,
      this.match,
      this.sMac,
      ...
      encryptKeys[0],
      encryptKeys[1],
      this.sEncPassFileName,
      this.nCipherBlockSizeInBytes,
      this.sPassword);

    const sPassword: string = 'ENCRYPTED_PASSWORD(file:' + this.sPassKeyFileName + ',file:' + this.sEncPassFileName + ')';
    ...
    try {
      ...
      const rows: any[] = cursor.fetchone();
      console.log(rows);
      cursor.close();
  ...
  private convertJavaNamestoNodeJs(sName: string, nKeySizeInBits: number = 0, sMode: string = ''): string {
  ...
  private createPasswordEncryptionKeyFile(sTransformation: string,
    sAlgorithm: string,
    sMatch: string,
    sMac: string,
  ...
  private createEncryptedPasswordFile(sTransformation: string,
    sAlgorithm: string,
    sMatch: string,
    sMac: string,
    sMode: string,
    ...
    const octetStringTag: any = 0x04; // The tag value for an octet string in der-format is 0x04
    ...
    const nodeAlgorithmName: string = this.convertJavaNamestoNodeJs(sAlgorithm, nKeySizeInBits, sMode);
  ...
  private loadPropertiesFile (sFileName: string): {} {
  ...
  private decryptPassword(sPassKeyFileName: string, sEncPassFileName: string): void {
    ...
    const mapPassKey: IMapPassKey = this.loadPropertiesFile(sPassKeyFileName);
    const mapEncPass: IMapEncPass = this.loadPropertiesFile(sEncPassFileName);
...
let exampleEncryptPassword: ExampleEncryptPassword = new ExampleEncryptPassword();
exampleEncryptPassword.setupAndRun();
