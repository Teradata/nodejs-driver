...
function createPasswordEncryptionKeyFile(
    ...
    const abyKey: Buffer = crypto_.randomBytes(nKeySizeInBytes);

    // Create MAC key
    const nMacBlockSizeBytes: number = 64;
    const abyMacKey: Buffer = crypto_.randomBytes(nMacBlockSizeBytes);
...
function createEncryptedPasswordFile(
    ...
    const abyIV: Buffer = crypto_.randomBytes(nCipherBlockSizeInBytes);
    ...
    const cipher: crypto_.Cipher = crypto_.createCipheriv(nodeAlgorithmName, abyKey, abyIV);
    ...
    const hmac: crypto_.Hmac = crypto_.createHmac(sMac, abyMacKey);
...
function loadPropertiesFile(sFileName: string): IMapPassKey | IMapEncPass {
...
function decryptPassword(sPassKeyFileName: string, sEncPassFileName: string): void {
    ...
    const hmac: crypto_.Hmac = crypto_.createHmac(sMac, abyMacKey); // Use MAC key
    hmac.update(abyContent);
    ...
    const decipher: crypto_.Decipher = crypto_.createDecipheriv(nodeAlgorithmName, abyKey, abyIV);
