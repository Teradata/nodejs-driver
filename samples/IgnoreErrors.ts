...
try {
	...
	try {
		...
		try {
			...
			if (ex instanceof teradatasql.OperationalError && ex.message.includes("[Error 3802]")) {
		...
		try {
			...
			if (ex instanceof teradatasql.OperationalError && ex.message.includes("[Error 3807]")) {
				console.log("Ignoring", ex.message.split("\n")[0]);
			} else {
				throw ex;
			}
		...
		try {
			...
			if (ex instanceof teradatasql.OperationalError && ex.message.includes("[Error 3824]")) {
				console.log("Ignoring", ex.message.split("\n")[0]);
			} else {
				throw ex;
			}
		...
		try {
			...
			if (ex instanceof teradatasql.OperationalError && ex.message.includes("[Error 5495]")) {
				console.log("Ignoring", ex.message.split("\n")[0]);
			} else {
				throw ex;
			}
